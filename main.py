...
def execute_cmd(cmd):
    ...
    if (cmd['cmd']=='show1' or cmd['cmd']=='show2') and (cmd['company']!=''):
        ...
        tts.play_sound(f'Для заказчика {cmd["company"]} найдено {num2text((len(list(filtered_df.iterrows()))))} позиций с отклонениями')
        for index,row in filtered_df.iterrows():
            item = config.convert_numbers_to_words(row['Синоним'])
            if cur_thread.stopped():
                return
...
def main():
    ...
    #va_respond('алиса добавь комментарий для строки один два ноль текст тест')
